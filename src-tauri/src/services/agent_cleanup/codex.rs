use super::shared::{
    agent_item_id, cleanup_result, count_to_u64, display_title, empty_scan_result,
    remove_jsonl_lines_by_str, safe_file_size, unique_ids,
};
use crate::models::{
...
pub(super) fn clean(home: &Path, ids: &[String]) -> Result<AgentCleanupResult, String> {
    ...
    for id in unique_ids(ids) {
        items.push(clean_one_thread(&codex_dir, &state_db, &id));
    }

    compact_databases(&codex_dir);
...
fn clean_one_thread(codex_dir: &Path, state_db: &Path, id: &str) -> AgentCleanupItemResult {
    ...
    match delete_thread_data(codex_dir, state_db, &thread) {
...
fn remove_index_lines(path: &Path, id_key: &str, id: &str) -> Result<(), String> {
    remove_jsonl_lines_by_str(path, id_key, id).map(|_| ())
}

fn compact_databases(codex_dir: &Path) {
...
mod tests {
    ...
    fn cleans_selected_thread_and_keeps_other_thread() {
        ...
        let result = clean(home, &[String::from("thread-a")]).expect("clean");
